...
class Form {
	...
	static function checkbox($params) {
		...
		$checkbox .= ' value="' . htmlentities($params['value']) . '"';
		$checkbox .= ' />';
	...
	static function dropdown($params) {
		...
		foreach ($params['values'] as $value => $label) {
			$dropdown .= '<option';
			$dropdown .= ' value="' . htmlentities($value) . '"';
	...
	static function hidden($params) {
		...
		if (!empty($params['value'])) $hidden .= ' value="' . htmlentities($params['value']) . '"';
		$hidden .= ' />';
		return $hidden;
	}
	...
	static function image($params) {
		...
		if (!empty($params['value'])) $image .= ' value="' . htmlentities($params['value']) . '"';
		$image .= ' />';
		return $image;
	}
	...
	static function input($params) {
		...
		if (!empty($params['value'])) $input .= ' value="' . htmlentities($params['value']) . '"';
		$input .= ' />';
		return $input;
	}
	...
	static function item($params) {
		...
		if (!array_key_exists('label', $params) || $params['label'] !== false) {
			$label_params = $params;
	...
	static function multiselect($params) {
		...
		foreach ($params['values'] as $value => $label) {
			$multiselect .= '<option';
			$multiselect .= ' value="' . htmlentities($value) . '"';
	...
	static function password($params) {
		...
		if (!empty($params['value'])) $password .= ' value="' . htmlentities($params['value']) . '"';
		$password .= ' />';
		return $password;
	}
	...
	static function radio($params) {
		...
		$radio .= ' value="' . htmlentities($params['value']) . '"';
		$radio .= ' />';
	...
	static function reset($params) {
		...
		$reset .= ' value="' . htmlentities($params['value']) . '"';
		$reset .= ' />';
		return $reset;
	}
	...
	static function submit($params) {
		...
		$submit .= ' value="' . htmlentities($params['value']) . '"';
		$submit .= ' />';
		return $submit;
	}
	...
	static function text($params) {
		...
		$text .= '>';
		if (!empty($params['value'])) $text .= htmlentities($params['value']);
		$text .= '</span>';
		return $text;
	}
	...
	static function textarea($params) {
		...
		$textarea .= '>';
		$textarea .= htmlentities($params['value']);
		$textarea .= '</textarea>';
		return $textarea;
	}
